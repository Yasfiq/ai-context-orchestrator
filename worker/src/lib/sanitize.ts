...
const INJECTION_PATTERNS: RegExp[] = [
  ...
  /<\|im_end\|>/i,
  /<\|endoftext\|>/i,
  /\[\/?(system|developer|assistant|context)\]/i,
  /(^|\n)\s*(system|developer|assistant)\s*:\s*/i,
  /\[INST\]/i,
  /<<SYS>>/i,
  /<\/?s>/i,

  // Advanced Jailbreak & Mode evasion
  /developer\s+mode\s+enabled/i,
  /unrestricted\s+mode/i,
  /do\s+anything\s+now/i,
  /bypass\s+all\s+(rules|restrictions|guardrails|filters)/i,
  /repeat\s+(everything|the\s+words)\s+above/i,
  /output\s+initialization\s+prompt/i,
  ...
  /hapus\s+semua\s+(batasan|filter|aturan)/i,
  /sekarang\s+kamu\s+bebas\s+dari\s+aturan/i,
];
